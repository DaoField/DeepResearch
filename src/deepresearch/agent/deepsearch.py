...
class DeepSearch:
    ...
    def _deep_search(
        ...
        if all_search:
            deep_search_result.all_knowledge = self._extract_all_knowledge(
                outline, all_search
            )
        colored_print("Learning above webpage", color="purple")
        ...
        colored_print("Learning done", color="purple")
        ...
        deep_search_result.children = self._deep_search(
            new_query, depth + 1, judge_results, outline, answer, pre_knowledge
        )
        return deep_search_result
            ...
            for result in search_result:
                ...
                if content_len + len(result.content) > extract_limit:
                    knowledge_results.extend(
                        self._extract_knowledge(outline, extract_search, extract_limit)
                    )
        ...
        try:
            text = llm(
                ...
                messages=apply_prompt_template(
                    ...
                    state={"chapter_outline": outline, "knowledge": documents},
                ),
            )
            if not text:
                return [], "<no answer>"
        ...
        match judge.name:
            case "completeness":
                prompt = apply_prompt_template(
                    ...
                    state={"chapter_outline": outline, "draft": answer},
                )
            case "freshness":
                prompt = apply_prompt_template(
            ...
            text = llm(
                ...
                messages=apply_prompt_template(
                    ...
                    state={
                        ...
                        "draft": answer,
                        "evaluation": "\n\n".join(
                            [eval.reason for eval in unpass_eval]
                        ),
                    },
                ),
            )
