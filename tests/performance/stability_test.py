...
class StabilityTest:
    ...
    def call_agent_sync(self, query: str) -> dict[str, Any]:
    ...
    def run_stability_test(
        self, duration: int = 3600, interval: int = 60
    ) -> dict[str, Any]:
        print(f"\n{'='*80}")
        print(f"运行长期稳定性测试")
        ...
        while time.time() < end_time:
            agent_result = self.call_agent_sync(query)
        ...
        response_times = [
            entry["agent_result"]["response_time"]
            for entry in self.monitoring_data
            if entry["agent_result"]["success"]
        ]
        success_count = sum(
            1 for entry in self.monitoring_data if entry["agent_result"]["success"]
        )
        error_count = sum(
            1
            for entry in self.monitoring_data
            if not entry["agent_result"]["success"]
        )
        ...
        cpu_percentages = [
            entry["system_status"]["cpu_percent"] for entry in self.monitoring_data
        ]
        memory_percentages = [
            entry["system_status"]["memory_percent"] for entry in self.monitoring_data
        ]
        memory_rss_values = [
            entry["system_status"]["memory_rss"] for entry in self.monitoring_data
        ]
        ...
        stats = {
            ...
            "success_rate": (
                success_count / len(self.monitoring_data)
    ...
    def generate_report(self, filename: str):
        with open(filename, "w", encoding="utf-8") as f:
            ...
            f.write("## 测试概述\n")
            f.write(
                "本报告记录了 DeepResearch 系统在长时间运行过程中的稳定性表现。\n\n"
            )
            ...
            for result in self.test_results:
                ...
                f.write(
                    f"- 平均内存使用率: {result['memory']['avg_percent']:.2f}%\n"
                )
                f.write(
                    f"- 最大内存使用率: {result['memory']['max_percent']:.2f}%\n"
                )
                f.write(
                    f"- 平均内存 RSS: {result['memory']['avg_rss'] / 1024 / 1024:.2f} MB\n"
                )
                f.write(
                    f"- 最大内存 RSS: {result['memory']['max_rss'] / 1024 / 1024:.2f} MB\n"
                )
                f.write(
                    f"- 内存泄漏检测: {'是' if result['memory_leak_detected'] else '否'}\n\n"
                )

            f.write("## 结论\n")
            f.write(
                "根据测试结果，系统在长时间运行过程中的稳定性表现如下：\n\n"
            )

            for result in self.test_results:
                f.write(
                    f"- 测试持续时间 {result['duration']} 秒：成功率 {result['success_rate']:.2f}，平均响应时间 {result['response_time']['avg']:.2f}s，平均 CPU 使用率 {result['cpu']['avg']:.2f}%，平均内存使用率 {result['memory']['avg_percent']:.2f}%，内存泄漏检测 {'是' if result['memory_leak_detected'] else '否'}\n"
                )
...
def main():
    ...
    test.run_stability_test(duration, interval)
