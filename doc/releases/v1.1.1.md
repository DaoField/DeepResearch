...
> **快速导航**: [文档索引](../index.rst) | [用户手册](../user_guide/user_guide.md) | [API文档](../api/api.md) | [架构设计](../architecture/architecture.md)
...
#### scikit-build-core 构建系统迁移

本次版本将构建系统从 `setuptools` 迁移至 `scikit-build-core`，以提供更现代化的构建体验和更好的跨平台支持。

**关键配置变更**:

| 配置项 | 变更前 (setuptools) | 变更后 (scikit-build-core) |
|--------|---------------------|---------------------------|
| 构建后端 | `setuptools.build_meta` | `scikit_build_core.build` |
| 构建依赖 | `setuptools>=61.0` | `scikit-build-core>=0.10` |
| 版本管理 | 静态版本 (`version = "x.x.x"`) | 动态版本 (`dynamic = ["version"]`) |

**pyproject.toml 关键配置**:

```toml
[build-system]
requires = ["scikit-build-core>=0.10", "setuptools_scm>=8.0"]
...
版本号现在通过 `setuptools_scm` 从 Git 标签自动提取，实现版本管理的自动化：

- **版本来源**: 从最近的 Git 标签（如 `v1.1.1`）自动提取版本号
- **开发版本**: 在开发分支上自动生成带有距离和提交哈希的开发版本（如 `1.1.1.dev3+g1234567`）
- **版本一致性**: 确保代码库、Git 标签和构建产物版本号完全一致
- **发布流程简化**: 只需打标签即可触发版本更新，无需手动修改代码

#### 构建系统变更的影响

| 影响范围 | 说明 |
|---------|------|
| **安装方式** | 支持 `pip install .` 和 `pip install -e .`（可编辑安装） |
| **构建工具** | 需要 `scikit-build-core>=0.10` 和 `setuptools_scm>=8.0` |
| **版本获取** | 通过 `importlib.metadata.version("deepresearch")` 获取版本号 |
| **向后兼容** | 所有公开 API 保持不变，现有代码无需修改 |
| **CMake 支持** | 为未来引入 C/C++ 扩展模块提供原生支持 |
